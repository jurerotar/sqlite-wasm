...
import sqlite3InitModule from '../../bin/sqlite3-core-opfs-wl-bundler-friendly.mjs';
...
self.onmessage = async () => {
  const filename = '/omit-api-opfs-wl-worker.sqlite3';
  ...
  try {
    ...
    try {
      db.exec('CREATE TABLE t(value); INSERT INTO t(value) VALUES (13)');
      db.close();
      db = undefined;

      db = new sqlite3.oo1.OpfsWlDb(filename, 'w');
      if (db.selectValue('SELECT value FROM t') !== 13) {
        throw new Error('OPFS WebLocks omit-api persistence check failed');
      }
