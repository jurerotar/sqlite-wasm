...
# usage:
#  docker run --rm -v $(pwd)/out:/out <image> build
#  docker run --rm -v $(pwd)/out:/out <image> shell   # for interactive debugging
...
# build wasm npm bundle
cd ext/wasm

# source emsdk again inside ext/wasm just to be safe
if [ -f /emsdk/emsdk_env.sh ]; then
  # shellcheck disable=SC1091
  source /emsdk/emsdk_env.sh
fi

log "Running make -j${BUILD_THREADS} npm (ext/wasm)"
make -j"${BUILD_THREADS}" npm
